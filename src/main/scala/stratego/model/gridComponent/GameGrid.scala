...
case class GameGrid(grid: Matrix[Field]) extends GridInterface{
  ...
  def createPlayableGrid(): GameGrid = {
    ...
    for {
      row <- 0 until 4
      col <- 0 until 10
    } gameGrid = copy(grid.replaceField(row, col, Field(FieldType.A_SIDE)))
    for {
      row <- 6 until 10
      col <- 0 until 10
    ...
    gameGrid = copy(grid.replaceField(5, 3, Field(FieldType.NO_FIELD)))
    gameGrid = copy(grid.replaceField(4, 6, Field(FieldType.NO_FIELD)))
    gameGrid = copy(grid.replaceField(4, 7, Field(FieldType.NO_FIELD)))
    gameGrid = copy(grid.replaceField(5, 6, Field(FieldType.NO_FIELD)))
    gameGrid = copy(grid.replaceField(5, 7, Field(FieldType.NO_FIELD)))
    gameGrid
  }
